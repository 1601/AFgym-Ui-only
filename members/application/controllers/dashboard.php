...
class Dashboard extends MY_Controller {
	...
	public function template($content, $data = ''){
		...
		$data["user_ids"] = $this->dashboard_model->get_dropdown('UserID', 'Lastname', 'userlist', array('isActive' => 1));
		$data["user_names"] =  $this->dashboard_model->get_dropdown('UserID', 'UserID', 'userlist', array('isActive' => 1));
		$data['count_all'] = $this->dashboard_model->get_count('userlist', array('isActive' => 1));
	...
	public function display($offset = 0, $sort_by = 'Username', $sort_order = 'asc', $limit = 40){
		...
		$results = $this->dashboard_model->get_users($data, $filters, $like, $string);
		...
		$data['match'] = $match;
		$data['string'] = $string;
		$this->template("dashboard/main", $data);
	}
	...
	public function add_users(){
		...
		$user_id = 1 + $this->dashboard_model->get_last_id('UserID', 'userlist');
		$this->dashboard_model->insert_row($insert, 'userlist');
	...
	public function add_function($function, $role){
		...
		$this->dashboard_model->insert_row($insert, 'RoleFunc');
		$this->session->set_flashdata('message', 'Successfully Edited Functionality' . $this->dashboard_model->get_roleDescription($role));
		redirect("dashboard");	
	}
	...
	public function edit_user($id){
			...
			$this->dashboard_model->update_row($insert, 'user', array('username' => $id));
	...
	public function deactivate_user($ID = "null"){
		...
		$data["isActive"] = 0;
		$this->dashboard_model->update_row($data, 'user', array('UserID' => $ID));
	...
	public function view_users(){
		...
		$data["id"] = $id;
		$data["user"] = $this->dashboard_model->get_user($id);
