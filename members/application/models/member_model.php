...
class member_model extends MY_Model{
    ...
    public function get_members($config, $filters, $like, $match){ //($config, $filters, $like, $match)
		$query = $this->db->select('members.*', FALSE)
					  ->from('members')
					  ->where($filters);
		...
		if ($num_rows == 0){
			...
			$ret['num_rows'] = $this->get_count('members', $filters, $like, $match); // ('Operators', $filters, $like, $match)
	...
	public function get_member($id){ 
		return $this->db->select('members.*', FALSE) 
						->from("members")
						->where('member_id', $id)
						->get()->result();
	}
