...
class Employee_model extends MY_Model{
    ...
    public function get_employees($config, $filters, $like, $match){ //($config, $filters, $like, $match)
		$query = $this->db->select('employees.*', FALSE)
					  ->from('employees')
					  ->where($filters);
		...
		if ($num_rows == 0){
			...
			$ret['num_rows'] = $this->get_count('employees', $filters, $like, $match); // ('Operators', $filters, $like, $match)
    ...
    public function get_payrolls($config, $filters, $like, $match){ //($config, $filters, $like, $match)
		
		$query = $this->db->select('payroll.*', FALSE)
					  ->from('payroll')
					  ->where($filters);
		if($like != '')$query = $this->db->like($like, $match);
		$query = $this->db->order_by($config['sort_by'], $config['sort_order'])
		...
		if ($num_rows == 0){
			...
			$ret['num_rows'] = $this->get_count('payroll', $filters, $like, $match); // ('Operators', $filters, $like, $match)
			$ret['rows'] = $query->result();
		}
	...
	public function get_employee($id){ 
		return $this->db->select('employees.*', FALSE) 
						->from("employees")
